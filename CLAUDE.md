...
- `db/`:持久化层。`db/index.ts` 单例连接(WAL)+ 建表 schema + 幂等增量迁移,各仓储全部预编译语句,业务层零 SQL。
  ...
  - `db/transactions.ts` — 收支流水 CRUD + 纠正 patch + 多维表格反向同步(`feishu_record_id` / `softDeleteByFeishuRecordId` / `findRecentEcho` 10min 回环兜底)+ 聚合查询(`totalsByProject` / `totalsByMonth` / `financeSummary` / `customerGroups` / `listCustomers`)
...
- `llm.ts`:@机器人问答入口 + 9 个工具的实现逻辑(create_todo/set_reminder/get_weather/record_income/record_expense/correct_transaction/query_finance/customer_groups/list_customers)。
...
- `feishu/`:
  ...
  - `bitable-todo.ts` — 待办事项写多维表格(`create_todo` 工具用;未配 `BITABLE_TODO_*` 则走纯提醒模式,只设渐进式提醒不写表)
- `routes/` — HTTP 路由
- `mastra/` — Mastra 智能体定义
...
| `create_todo` | 待办事项(渐进式 3 轮提醒) | `reminders` 表(batch_id/round)+ 可选写待办多维表格 |
...
3. **回环兜底**:刚正向写入但 `feishu_record_id` 还没回写的行(10min 窗口),用 `findRecentEcho`(方向/金额/币种/日期全等)匹配关联,避免被当成表格手填再插一条
...
| `BITABLE_TODO_APP_TOKEN` | 否 | 待办事项多维表格 app_token(留空 = `create_todo` 走纯提醒模式,只设渐进式提醒不写表) |
| `BITABLE_TODO_TABLE_ID` | 否 | 待办事项多维表格 table_id |
| `BITABLE_TODO_LINK` | 否 | 待办表格链接(创建待办后回复里附上) |
| `CORS_ORIGINS` | 否 | CORS 允许来源白名单(逗号分隔)。留空 = 不挂 CORS、走浏览器默认同源(跨域被拒);接口含敏感流水,务必按需收紧 |
| `BITABLE_DEBUG_EVENT` | 否 | 反向同步事件原始 payload 落盘开关,设 `1` 开启(联调用,生产留空) |
