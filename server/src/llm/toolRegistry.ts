 ...
 * 用户等图等不到。命中即追加系统核对戳穿(llm.ts 还会据此触发一轮纠偏重试,强制真调工具)。
 *
 * 简繁都要兜住(LLM 输出简繁不稳定),且要覆盖"已经生成"这种带副词的完成态变体:
 *   - "海报已经生成" → "海报已经" 命中;"已生成" 不命中(中间隔了"经")
 *   - "海報已經生成" → "海報已經" 命中;"已生成" 不命中(中间隔了"經")
 */
export const CLAIMS_PLATES = /生成出图|已出图|出图啦|已生成|已经生成|已經生成|帮你生成|幫你生成|海报已|海报已经|海報已|海報已經|已發圖|已发图|已發送海報|已发送海报|发送了海报|发了海报|發了海報|发出图|發出圖|出图给你|出圖給你/
