...
const GENERATE_DAILY_PLATES_TOOL = {
  ...
  description: '生成「每日靓号推荐」海报图并发到群里。当用户发来口岸+一批两地车牌清单(文字/截图/文档),且候选车牌**至少 2 个**时才调用;只有 1 个候选就不要调,直接回复让对方补齐。你负责:①提取口岸;②从香港人/粤语角度挑出最佳 2 个号(忌 4 及不雅谐音,好 8/9/6 尾、豹子/顺子/一路發类组合,结合批文情况);③把全部候选和你的 2 个精选都通过参数传进来。卡片上的日期由系统自动取今天,你不要传任何日期。',
  input_schema: {
    type: 'object' as const,
    properties: {
      port: {
...
export interface LlmContext {
  ...
  /** 消息里的文件(file 类型消息,如车牌清单文档) */
  voucherFileKeys?: { key: string; name: string }[]
  /** 每个文件对应的 message_id(同 imageMessageIds 道理) */
...
- 生成每日靓号推荐:用户发来「口岸 + 一批两地车牌清单」(可能是一段文字、一张截图、一个文档)并 @你,意图就是让你挑靓号出图 → 调 generate_daily_plates。判断特征:内容里有口岸名(莲塘/深圳湾/港珠澳大桥/文锦渡/皇岗/罗湖/福田/沙头角等)和 3 个以上车牌号(粤Z 开头)。
  ...
  · **从香港人/粤语角度挑 2 个最佳靓号**放进 picks(第 1 个上「特選」章)。价值排序(从高到低):
    ...
    - 结合批文情况:同等靓度优先「有批文」或批文日期更近的(上牌快);这层权衡写进 reason。
    - reason 用一句话粤语生意口吻,如「一路發,尾 8 大吉,仲有批文上牌快」。
...
async function buildUserContent(question: string, ctx: LlmContext): Promise<Anthropic.MessageParam['content']> {
  ...
  // 消息里带文档(file 类型):下载并提取纯文字拼进正文,LLM 才"读得懂"文档
  if (ctx.voucherFileKeys?.length) {
    for (let i = 0; i < ctx.voucherFileKeys.length; i++) {
      ...
      if (!buf) {
        text += `\n\n【文档:${f.name}】(下载失败,无法读取内容)`
        continue
      }
      const docText = extractDocumentText(f.name, buf)
      text += docText
        ? `\n\n【文档:${f.name} 的内容】\n${docText}`
        : `\n\n【文档:${f.name}】(暂不支持该格式的解析,仅支持 docx/txt/md/csv)`
    }
  }
